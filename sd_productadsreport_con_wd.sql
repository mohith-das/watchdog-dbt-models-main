...
with cte as (
{% set tab = dbt_utils.get_column_values(table=ref('sd_productadsreport_info_wd'),column='tables',default=[]) %}
{% for i in tab %}
    {% set id =i.split('.')[2].split('_')[0] %}
    select * except(row_num),
    {{ dbt_utils.surrogate_key([ 'reportDate','client_name', 'countryName', 'accountId', 'profileId', 'campaignId','adId','adGroupId','asin','sku'])}} as primary_key
    from (
