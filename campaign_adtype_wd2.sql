...
ComboTable as 
(
    select * from {{ ref("campaign_adtype_wd1") }}
),
...
pre_final_1w as (
    ...
    on ct1.client_name = ct2.client_name and 
        ...
        ct1.campaignId = ct2.campaignId and
       ct1.campaignName = ct2.campaignName and
...
pre_final_30d as (
    ...
    group by date, client_name, countryname, campaignId, campaignName, adtype, 
...
pre_final as (
    ...
    {% endfor %}
    from pre_final_1w 
    full join pre_final_2w using (date, client_name, countryname, campaignId, campaignName, adtype)
    full join pre_final_3w using (date, client_name, countryname, campaignId, campaignName, adtype)
    full join pre_final_4w using (date, client_name, countryname, campaignId, campaignName, adtype)
    full join pre_final_52w using (date, client_name, countryname, campaignId, campaignName, adtype)
    full join pre_final_30d using (date, client_name, countryname, campaignId, campaignName, adtype)
    full join pre_final_90d using (date, client_name, countryname, campaignId, campaignName, adtype)
),
thresholdstable as (
    select *
    from
...
or AdImpressions_Status != 0 or ACOS_status != 0 or  AdSpend_Status != 0
