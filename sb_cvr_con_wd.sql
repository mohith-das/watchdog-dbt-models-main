...
{% set tab = dbt_utils.get_column_values(table=ref('sb_cvr_info_wd'),column='tables',default=[]) %}

with cte as (
{% for i in tab %}
    {% set id =i.split('.')[2].split('_')[0] %}
    SELECT * except(row_num),
{{ dbt_utils.surrogate_key(['reportDate','client_name' ,'countryName','profileId','accountId','campaignId'])}} as primary_key
    from (
    select 
'{{id}}' as client_name,
profileId,
